...
// Sprite width/height match the PC dashboard canvas.
constexpr int WEATHER_SCREEN_WIDTH = 320;
constexpr int WEATHER_SCREEN_HEIGHT = 170;
...
constexpr uint8_t WEATHER_BACKLIGHT_PIN = 38;
constexpr uint8_t WEATHER_BACKLIGHT_CHANNEL = 4;
...
// Optional brightness buttons (disabled by default, set to GPIO if wired)
